...
class ObjOperacionalsController extends Controller
{
    ...
    public function index(Objoperacional $objoperacional,Anoreporte $anoreporte)
    {
        $anoreporteid = $anoreporte->id;
        $objoperacionalid = $objoperacional->id;
        $fecha_actual = date('Y-m-d');

        $asignacions = Asignacion::where('objoperacional_id',$objoperacionalid)
        ...
        foreach ($asignacions as $asignacion){
            ...
            $plan_recopilacion_d = ((($dias_real_recopilacion_d) * 100) / ($asignacion->plan_dias_recopilacion)) * 0.20;
            }
            ...
            $plan_inf_d = ((($dias_real_inf_d) * 100) / ($asignacion->plan_dias_inf)) * 0.40;
            }
            ...
            $plan_divulgacion_d = ((($dias_real_divulgacion_d) * 100) / ($asignacion->plan_dias_divulgacion)) * 0.20;
            }
            ...
            $plan_carga_d = ((($dias_real_carga_d) * 100) / ($asignacion->plan_dias_carga)) * 0.10;
            }
        ...
        return view('reportes.objoperacionals',compact('plan_total','real_total','desviacion','cumplimiento','anoreporteid','reportegeneral'));
    }
