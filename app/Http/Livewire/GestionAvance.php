...
{ 
    ...
    public function render()
    {
       ...
       foreach ($asignacions as $asignacion){
            ...
            if(Carbon::parse($asignacion->fecha_conformacion_i) < Carbon::parse($this->fecha_actual)){
            ...
            if(Carbon::parse($asignacion->fecha_recopilacion_i) < Carbon::parse($this->fecha_actual)){
            $dias_real_recopilacion = Carbon::parse($asignacion->fecha_recopilacion_i)->diffInDays(Carbon::parse($this->fecha_actual));
            $plan_recopilacion = ((($dias_real_recopilacion) * 100) / ($asignacion->plan_dias_recopilacion)) * 0.20;
            }
            if(Carbon::parse($asignacion->fecha_inf_i) < Carbon::parse($this->fecha_actual)){
            $dias_real_inf = Carbon::parse($asignacion->fecha_inf_i)->diffInDays(Carbon::parse($this->fecha_actual));
            $plan_inf = ((($dias_real_inf) * 100) / ($asignacion->plan_dias_inf)) * 0.40;
            }
            if(Carbon::parse($asignacion->fecha_divulgacion_i) < Carbon::parse($this->fecha_actual)){
            $dias_real_divulgacion = Carbon::parse($asignacion->fecha_divulgacion_i)->diffInDays(Carbon::parse($this->fecha_actual));
            $plan_divulgacion = ((($dias_real_divulgacion) * 100) / ($asignacion->plan_dias_divulgacion)) * 0.20;
            }
            if(Carbon::parse($asignacion->fecha_carga_i) < Carbon::parse($this->fecha_actual)){
            $dias_real_carga = Carbon::parse($asignacion->fecha_carga_i)->diffInDays(Carbon::parse($this->fecha_actual));
            $plan_carga = ((($dias_real_carga) * 100) / ($asignacion->plan_dias_carga)) * 0.10;
            }
